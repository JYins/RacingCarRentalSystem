<template>
  <div class="addEdit-block">
    <el-form class="detail-form-content" ref="ruleForm" :model="ruleForm" :rules="rules" label-width="80px"
      ...
      <el-row>
        <el-col :span="12">
          <el-form-item class="input" v-if="type != 'info'" label="Area" prop="area">
            <el-input v-model="ruleForm.area" placeholder="Area" clearable :readonly="ro.area"></el-input>
          </el-form-item>
          <div v-else>
            <el-form-item class="input" label="Area" prop="area">
              <el-input v-model="ruleForm.area" placeholder="Area" readonly></el-input>
            </el-form-item>
          </div>
        </el-col>
      </el-row>
...
export default {
  data() {
    ...
    return {
      ...
      rules: {
        area: [
          { required: true, message: 'Area cannot be empty', trigger: 'blur' },
        ],
      }
    };
  },
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `dispenseArea/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {

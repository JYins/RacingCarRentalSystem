<template>
  <div class="main-content">
    ...
    <div v-if="showFlag">
      <el-form :inline="true" :model="searchForm" class="form-content">
        ...
        <el-row class="ad"
          ...
          <el-form-item>
            <el-button
              v-if="isAuth('carReturn', 'Add') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 1"
                @click="addOrUpdateHandler()">{{ contents.btnAdAllFont == 1 ? ' Add' : '' }}</el-button>
            <el-button
              v-if="isAuth('carReturn', 'Add') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 2"
              @click="addOrUpdateHandler()">{{ contents.btnAdAllFont == 1 ? ' Add' : '' }}</el-button>
            <el-button v-if="isAuth('carReturn', 'Add') && contents.btnAdAllIcon == 0" @click="addOrUpdateHandler()">{{
              contents.btnAdAllFont == 1 ? ' Add' : '' }}</el-button>
            <el-button
              v-if="isAuth('carReturn', 'Delete') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 1 && contents.tableSelection"
              :disabled="dataListSelections.length <= 0" icon="el-icon-delete" @click="deleteHandler()">{{
                contents.btnAdAllFont == 1 ? ' Delete' : '' }}</el-button>
            <el-button
              v-if="isAuth('carReturn', 'Delete') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 2 && contents.tableSelection"
              :disabled="dataListSelections.length <= 0" @click="deleteHandler()">{{ contents.btnAdAllFont == 1 ? ' Delete' :
                ''
              }}</el-button>
            <el-button v-if="isAuth('carReturn', 'Delete') && contents.btnAdAllIcon == 0 && contents.tableSelection"
              :disabled="dataListSelections.length <= 0" @click="deleteHandler()">{{ contents.btnAdAllFont == 1 ? ' Delete' :
                ''
              }}</el-button>
      ...
      <div class="table-content">
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
          ...
          v-if="isAuth('carReturn', 'View')" :data="dataList" v-loading="dataListLoading"
          ...
          <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="returnTime"
            header-align="center" width="200" label="Return Time">
            <template slot-scope="scope">
          ...
          <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="mobilePhone" width="200"
          ...
          <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="reviewResponse"
            header-align="center" label="Review Response" width="150">
            <template slot-scope="scope">
              <el-button @click="timeoutPay(scope.row)" style="background-color: red;color: aliceblue !important;"
                v-if="scope.row.reviewResponse == 'Order Timeout'">Timeout Pay</el-button>
              <span v-else>{{ scope.row.reviewResponse }}</span>
            </template>
          </el-table-column>
          <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="isReviewed"
            header-align="center" label="Review Status" width="160px">
            <template slot-scope="scope">
              <div v-if="scope.row.isReviewed == 'Yes'" class="agree"> <span
                  style="margin-right:0px">Approved</span></div>
              <div v-else class="disagree"> <span style="margin-right:0px">Not Approved</span></div>
            </template>
          </el-table-column>
          <!-- <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" width="160"
            v-if="isAuth('carReturn', '审核')" prop="isReviewed" header-align="center" label="审核">
            <template slot-scope="scope">
              <el-button type="primary" @click="shDialog(scope.row)"> 审核</el-button>
            </template>
          </el-table-column> -->
          <el-table-column width="420" :align="contents.tableAlign" header-align="center" label="Actions" fixed="right">
            <template slot-scope="scope">
              <el-button v-if="isAuth('carReturn', 'Review')" type="primary" @click="shDialog(scope.row)"> Review</el-button>
              <el-button
                v-if="isAuth('carReturn', 'View') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1"
                   @click="addOrUpdateHandler(scope.row.id, 'info')">{{
                  contents.tableBtnFont == 1 ? ' Details' : '' }}</el-button>
              <el-button
                v-if="isAuth('carReturn', 'View') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2"
                  @click="addOrUpdateHandler(scope.row.id, 'info')">{{ contents.tableBtnFont == 1 ? ' Details' : ''
                }}</el-button>
              <el-button v-if="isAuth('carReturn', 'View') && contents.tableBtnIcon == 0"  
                @click="addOrUpdateHandler(scope.row.id, 'info')">{{ contents.tableBtnFont == 1 ? ' Details' : '' }}</el-button>
              <el-button
                v-if="isAuth('carReturn', 'Edit') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1"
                    @click="addOrUpdateHandler(scope.row.id)">{{ contents.tableBtnFont ==
                  1 ? 'Edit' : '' }}</el-button>
              <el-button
                v-if="isAuth('carReturn', 'Edit') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2"
                  @click="addOrUpdateHandler(scope.row.id)">{{ contents.tableBtnFont == 1 ? 'Edit' : '' }}<i
                  class="el-icon-edit el-icon--right" /></el-button>
              <el-button v-if="isAuth('carReturn', 'Edit') && contents.tableBtnIcon == 0"  
              ...
              <el-button
                v-if="isAuth('carReturn', 'Delete') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1"
                icon="el-icon-delete"   @click="deleteHandler(scope.row.id)">{{ contents.tableBtnFont ==
                  1 ? ' Delete' : '' }}</el-button>
              <el-button
                v-if="isAuth('carReturn', 'Delete') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2"
                  @click="deleteHandler(scope.row.id)">{{ contents.tableBtnFont == 1 ? ' Delete' : '' }}<i
                  class="el-icon-delete el-icon--right" /></el-button>
              <el-button v-if="isAuth('carReturn', 'Delete') && contents.tableBtnIcon == 0"  
    ...
    <el-dialog title="Review" :visible.sync="sfshVisiable" width="50%">
      <el-form ref="form" :model="form" label-width="80px">
        ...
        <el-form-item label="Content">
          <!-- <editor style="min-width: 200px; max-width: 600px;" v-model="shForm.reviewResponse" class="editor"
            action="file/upload">
          </editor> -->
    ...
    <el-dialog title="Timeout Payment" :visible.sync="timeoutDialog" width="80%">
      <div class="container">
        <el-alert title="Please confirm order info before payment" :closable="false"></el-alert>

        <div class="pay-type-content ">
          <div class="pay-type-item">
            <el-radio v-model="payType" label="Credit Card (Visa/MC)"></el-radio>
            <span style="font-size:24px;">💳</span>
          </div>
          <div class="pay-type-item">
            <el-radio v-model="payType" label="PayPal"></el-radio>
            <span style="font-size:24px;">🅿️</span>
          </div>
          <div class="pay-type-item">
            <el-radio v-model="payType" label="Apple Pay"></el-radio>
            <span style="font-size:24px;">🍎</span>
          </div>
          <div class="pay-type-item">
            <el-radio v-model="payType" label="Google Pay"></el-radio>
            <span style="font-size:24px;">📱</span>
          </div>
          <div class="pay-type-item">
            <el-radio v-model="payType" label="Interac e-Transfer"></el-radio>
            <span style="font-size:24px;">🏦</span>
          </div>
          <div class="pay-type-item">
            <el-radio v-model="payType" label="Debit Card"></el-radio>
            <span style="font-size:24px;">💵</span>
          </div>
        </div>
      </div>
      <span slot="footer" class="dialog-footer">
        <el-button @click="timeoutDialog =false"> Cancel</el-button>
        <el-button @click="submitTimeoutPay" type="primary"> Confirm</el-button>
      </span>
    </el-dialog>
...
export default {
  ...
  methods: {
    submitTimeoutPay() {
      ...
      this.$confirm(`Confirm payment?`, "Hint", {
        confirmButtonText: "Confirm",
        cancelButtonText: "Cancel",
        type: "warning"
      }).then(() => {
        this.timeoutForm.reviewResponse ="Timeout order paid"
        this.timeoutForm.isReviewed ="Yes"
        this.$http({
          url: "carReturn/update",
          method: "post",
          data: this.timeoutForm
        }).then(({ data }) => {
          if (data && data.code === 0) {
            this.$message({
              message: "操作成功",
    ...
    timeoutPay(row) {
      this.timeoutForm = row
      console.log(this.timeoutForm)
    ...
    shDialog(row) {
      ...
      if (row) {
        this.shForm = {
          ...
          returnContent: row.returnContent,
    ...
    shHandler() {
      this.$confirm(`Confirm action?`, "Hint", {
        ...
        this.shForm.isReturn = this.shForm.isReviewed
        this.$http({
          url: "carReturn/update",
          method: "post",
          data: this.shForm

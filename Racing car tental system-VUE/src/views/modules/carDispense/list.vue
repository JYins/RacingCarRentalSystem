<template>
  <div class="main-content">
    ...
    <div v-if="showFlag">
      <el-form :inline="true" :model="searchForm" class="form-content">
        <el-row :gutter="20" class="slt"
          ...
          <el-form-item :label="contents.inputTitle == 1 ? 'License Plate' : ''">
            <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 1"  
              v-model="searchForm.licensePlateNumber" placeholder="License Plate" clearable></el-input>
            <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 2"  
              v-model="searchForm.licensePlateNumber" placeholder="License Plate" clearable></el-input>
            <el-input v-if="contents.inputIcon == 0" v-model="searchForm.licensePlateNumber" placeholder="License Plate"
              clearable></el-input>
          </el-form-item>
          <el-form-item :label="contents.inputTitle == 1 ? 'Dispense Location' : ''">
            <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 1"  
              v-model="searchForm.dispenseLocation" placeholder="Dispense Location" clearable></el-input>
            <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 2"  
              v-model="searchForm.dispenseLocation" placeholder="Dispense Location" clearable></el-input>
            <el-input v-if="contents.inputIcon == 0" v-model="searchForm.dispenseLocation" placeholder="Dispense Location"
              clearable></el-input>
          </el-form-item>
          <el-form-item>
        ...
        <el-row class="ad"
          ...
          <el-form-item>
            <el-button
              v-if="isAuth('carDispense', 'Add') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 1"
                 @click="addOrUpdateHandler()">{{ contents.btnAdAllFont == 1 ? ' Add' : ''
              }}</el-button>
            <el-button
              v-if="isAuth('carDispense', 'Add') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 2"
               @click="addOrUpdateHandler()">{{ contents.btnAdAllFont == 1 ? ' Add' : '' }}</el-button>
            <el-button v-if="isAuth('carDispense', 'Add') && contents.btnAdAllIcon == 0" 
              @click="addOrUpdateHandler()">{{ contents.btnAdAllFont == 1 ? ' Add' : '' }}</el-button>
            <el-button
              v-if="isAuth('carDispense', 'Delete') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 1 && contents.tableSelection"
              :disabled="dataListSelections.length <= 0"  icon="el-icon-delete" @click="deleteHandler()">{{
                contents.btnAdAllFont == 1 ? ' Delete' : '' }}</el-button>
            <el-button
              v-if="isAuth('carDispense', 'Delete') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 2 && contents.tableSelection"
              :disabled="dataListSelections.length <= 0"  @click="deleteHandler()">{{ contents.btnAdAllFont
                == 1 ? ' Delete' : '' }}</el-button>
            <el-button v-if="isAuth('carDispense', 'Delete') && contents.btnAdAllIcon == 0 && contents.tableSelection"
              :disabled="dataListSelections.length <= 0"  @click="deleteHandler()">{{ contents.btnAdAllFont
                == 1 ? ' Delete' : '' }}</el-button>
      ...
      <div class="table-content">
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
          ...
          v-if="isAuth('carDispense', 'View')" :data="dataList" v-loading="dataListLoading"
          ...
          <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="bodyColor"
            header-align="center" label="Body Color">
            <template slot-scope="scope">
              {{ scope.row.bodyColor }}
            </template>
          </el-table-column>
          ...
          <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="dispenseRegion"
            header-align="center" label="Dispense Area">
            <template slot-scope="scope">
              {{ scope.row.dispenseRegion }}
            </template>
          </el-table-column>
          <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="dispenseLocation"
            header-align="center" label="Dispense Location">
            <template slot-scope="scope">
              {{ scope.row.dispenseLocation }}
            </template>
          </el-table-column>
          <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="dispenseTime"
            header-align="center" label="Dispense Time" width="200">
            <template slot-scope="scope">
              {{ scope.row.dispenseTime }}
            </template>
          </el-table-column>
          <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="carCondition"
            header-align="center" label="Car Status" width="160">
            <template slot-scope="scope">
              <div v-if="scope.row.carCondition == 'In Use'" class="disagree"> In Use</div>
              <div v-else class="agree"> Available</div>
          ...
          <el-table-column width="300" :align="contents.tableAlign" header-align="center" label="Actions" fixed="right">
            <template slot-scope="scope">
              <el-button
                v-if="isAuth('carDispense', 'View') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1"
                 icon="el-icon-tickets"   @click="addOrUpdateHandler(scope.row.id, 'info')">{{
                  contents.tableBtnFont == 1 ? ' Details' : '' }}</el-button>
              <el-button
                v-if="isAuth('carDispense', 'View') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2"
                   @click="addOrUpdateHandler(scope.row.id, 'info')">{{ contents.tableBtnFont ==
                  1 ? ' Details' : '' }}</el-button>
              <el-button v-if="isAuth('carDispense', 'View') && contents.tableBtnIcon == 0"   
                @click="addOrUpdateHandler(scope.row.id, 'info')">{{ contents.tableBtnFont == 1 ? ' Details' : '' }}</el-button>
              <el-button
                v-if=" scope.row.carCondition != '使用' && isAuth('carDispense', 'Use') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1"
                 icon="el-icon-tickets"  
                @click="useOrderCrossAddOrUpdateHandler(scope.row, 'cross')">{{ contents.tableBtnFont == 1 ? ' Use' : ''
                }}</el-button>
              <el-button
                v-if="scope.row.carCondition != '使用' && isAuth('carDispense', 'Use') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2"
                   @click="useOrderCrossAddOrUpdateHandler(scope.row, 'cross')">{{
                  contents.tableBtnFont == 1 ? ' Use' : '' }}</el-button>
              <el-button v-if="isAuth('carDispense', 'Use') && contents.tableBtnIcon == 0"   
                @click="useOrderCrossAddOrUpdateHandler(scope.row, 'cross')">{{ contents.tableBtnFont == 1 ? ' Use' : ''
                }}</el-button>
              <el-button
                v-if="isAuth('carDispense', 'Edit') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1"
                     @click="addOrUpdateHandler(scope.row.id)">{{
                  contents.tableBtnFont == 1 ? 'Edit' : '' }}</el-button>
              <el-button
                v-if="isAuth('carDispense', 'Edit') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2"
                   @click="addOrUpdateHandler(scope.row.id)">{{ contents.tableBtnFont == 1 ? 'Edit' : ''
                }}<i class="el-icon-edit el-icon--right" /></el-button>
              <el-button v-if="isAuth('carDispense', 'Edit') && contents.tableBtnIcon == 0"   
              ...
              <el-button
                v-if="isAuth('carDispense', 'Delete') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1"
                 icon="el-icon-delete"   @click="deleteHandler(scope.row.id)">{{
                  contents.tableBtnFont == 1 ? ' Delete' : '' }}</el-button>
              <el-button
                v-if="isAuth('carDispense', 'Delete') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2"
                   @click="deleteHandler(scope.row.id)">{{ contents.tableBtnFont == 1 ? ' Delete' : '' }}</el-button>
              <el-button v-if="isAuth('carDispense', 'Delete') && contents.tableBtnIcon == 0"   
                @click="deleteHandler(scope.row.id)">{{ contents.tableBtnFont == 1 ? ' Delete' : '' }}</el-button>
            </template>
          </el-table-column>
        </el-table>
    ...
    <useOrderCrossAddOrUpdate v-if="useOrderCrossAddOrUpdateFlag" :parent="this" ref="useOrderCrossaddOrUpdate">
    </useOrderCrossAddOrUpdate>
...
import useOrderCrossAddOrUpdate from "../useOrder/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      useOrderCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    useOrderCrossAddOrUpdateHandler(row, type) {
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.useOrderCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj', row);
      this.$storage.set('crossTable', 'carDispense');
      this.$nextTick(() => {
        this.$refs.useOrderCrossaddOrUpdate.init(row.id, type);
      });
    },
    init() {
    },
    ...
    getDataList() {
      ...
      if (this.searchForm.dispenseLocation != '' && this.searchForm.dispenseLocation != undefined) {
        params['dispenseLocation'] = '%' + this.searchForm.dispenseLocation + '%'
      }
      this.$http({
        url: "carDispense/page",
        method: "get",
        params: params
      }).then(({ data }) => {
        if (data && data.code === 0) {

    ...
    <div v-if="showFlag">
      <el-form :inline="true" :model="searchForm" class="form-content">
        <el-row :gutter="20" class="slt"
          ...
          <el-form-item :label="contents.inputTitle == 1 ? 'Username' : ''">
            <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 1" v-model="searchForm.username"
              placeholder="Username" clearable></el-input>
            <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 2" v-model="searchForm.username"
              placeholder="Username" clearable></el-input>
        ...
        <el-row class="ad"
          ...
          <el-form-item>
            <el-button v-if="isAuth('admin', 'Add') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 1"
                @click="addOrUpdateHandler()">{{ contents.btnAdAllFont == 1 ? ' Add' : '' }}</el-button>
            <el-button v-if="isAuth('admin', 'Add') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 2"
              @click="addOrUpdateHandler()">{{ contents.btnAdAllFont == 1 ? ' Add' : '' }}</el-button>
            <el-button v-if="isAuth('admin', 'Add') && contents.btnAdAllIcon == 0" @click="addOrUpdateHandler()">{{
              contents.btnAdAllFont == 1 ? ' Add' : '' }}</el-button>
            <el-button
              v-if="isAuth('admin', 'Delete') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 1 && contents.tableSelection"
              :disabled="dataListSelections.length <= 0" icon="el-icon-delete" @click="deleteHandler()">{{
                contents.btnAdAllFont == 1 ? 'Delete' : '' }}</el-button>
            <el-button
              v-if="isAuth('admin', 'Delete') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 2 && contents.tableSelection"
              :disabled="dataListSelections.length <= 0" @click="deleteHandler()">{{ contents.btnAdAllFont == 1 ? 'Delete' : ''
              }}</el-button>
            <el-button v-if="isAuth('admin', 'Delete') && contents.btnAdAllIcon == 0 && contents.tableSelection"
              :disabled="dataListSelections.length <= 0" @click="deleteHandler()">{{ contents.btnAdAllFont == 1 ? 'Delete' : ''
              }}</el-button>
      ...
      <div class="table-content">
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
          ...
          v-if="isAuth('admin', 'View')" :data="dataList" v-loading="dataListLoading"
          ...
          <el-table-column width="420" :align="contents.tableAlign" header-align="center" label="Actions" fixed="right">
            <template slot-scope="scope">
              <el-button v-if="isAuth('admin', 'View') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1"
                icon="el-icon-tickets" @click="addOrUpdateHandler(scope.row.id, 'info')">{{ contents.tableBtnFont ==
                  1 ? 'Details' : '' }}</el-button>
              <el-button v-if="isAuth('admin', 'View') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2"
                @click="addOrUpdateHandler(scope.row.id, 'info')">{{ contents.tableBtnFont == 1 ? 'Details' : '' }}</el-button>
              <el-button v-if="isAuth('admin', 'View') && contents.tableBtnIcon == 0"
                @click="addOrUpdateHandler(scope.row.id, 'info')">{{ contents.tableBtnFont == 1 ? 'Details' : '' }}</el-button>
              <el-button v-if="isAuth('admin', 'Edit') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1"
                @click="addOrUpdateHandler(scope.row.id)">{{ contents.tableBtnFont == 1 ? 'Edit' : '' }}</el-button>
              <el-button v-if="isAuth('admin', 'Edit') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2"
                @click="addOrUpdateHandler(scope.row.id)">{{ contents.tableBtnFont == 1 ? 'Edit' : '' }}<i
                  class="el-icon-edit el-icon--right" /></el-button>
              <el-button v-if="isAuth('admin', 'Edit') && contents.tableBtnIcon == 0"
                @click="addOrUpdateHandler(scope.row.id)">{{ contents.tableBtnFont == 1 ? 'Edit' : '' }}</el-button>

              <el-button v-if="isAuth('admin', 'Delete') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1"
                icon="el-icon-delete" @click="deleteHandler(scope.row.id)">{{ contents.tableBtnFont == 1 ? 'Delete' : ''
                }}</el-button>
              <el-button v-if="isAuth('admin', 'Delete') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2"
                @click="deleteHandler(scope.row.id)">{{ contents.tableBtnFont == 1 ? 'Delete' : '' }}<i
                  class="el-icon-delete el-icon--right" /></el-button>
              <el-button v-if="isAuth('admin', 'Delete') && contents.tableBtnIcon == 0" @click="deleteHandler(scope.row.id)">{{
